...
func (db *Database) UpdateUser(ctx context.Context, user *model.User) (*model.User, error) {
	var u model.User
	row := db.Client.QueryRowContext(ctx, `UPDATE users SET username=$2, password= $3, email= $4 WHERE id= $1 RETURNING id, username, password, email, create_at`, user.ID, user.Username, user.Password, user.Email, user.CreatedAt)
