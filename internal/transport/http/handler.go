...
func (h *Handler) Routers() {
	...
	h.Router.HandleFunc("/api/v1/user", h.GetUser).Methods("GET")
	h.Router.HandleFunc("/api/v1/user", h.PostUser).Methods("POST")
	h.Router.HandleFunc("/api/v1/user", JWTAuth(h.UpdateUser)).Methods("PUT")
	h.Router.HandleFunc("/api/v1/user", JWTAuth(h.DeleteUser)).Methods("DELETE")
